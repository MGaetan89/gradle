...
Converting a `Configuration` to a simple `Set<File>`s will also cause the loss of any implicit task dependencies it carries.

You should avoid using these methods when configuring your build and stick to the methods defined directly on the Gradle interfaces.
Instead, use <<lazy_configuration.adoc#working_with_files_in_lazy_properties,lazy types and APIs>> that defer resolution to wire task dependencies and inputs correctly.
...
====
include::sample[dir="snippets/bestPractices/avoidJdkCollectionApiOnFileCollections-avoid/kotlin",files="build.gradle.kts[tags=avoid-this]"]
include::sample[dir="snippets/bestPractices/avoidJdkCollectionApiOnFileCollections-avoid/groovy",files="build.gradle[tags=avoid-this]"]
====

<1> *`isEmpty()` Causes Resolution*: Many seemingly harmless Collections API methods like `isEmpty()` can cause Gradle to resolve dependencies.
<2> *Accessing Files directlry*: Using `getFiles()` to access the files in a `Configuration` will also cause Gradle to resolve the file collection.
...
====
include::sample[dir="snippets/bestPractices/avoidJdkCollectionApiOnFileCollections-do/kotlin",files="build.gradle.kts[tags=do-this]"]
include::sample[dir="snippets/bestPractices/avoidJdkCollectionApiOnFileCollections-do/groovy",files="build.gradle[tags=do-this]"]
====
